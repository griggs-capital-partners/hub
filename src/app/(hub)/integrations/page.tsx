  ...
  const integrations = [
    {
      id: "github",
      name: "GitHub",
      description: "Connect repositories to your workspace. Sync issues, track progress, and tie team members to their GitHub identities.",
      href: "/integrations/github",
      status: "active" as const,
      icon: (
        <svg viewBox="0 0 16 16" fill="currentColor" className="w-6 h-6 text-[#F0F0F0]">
...

    {
      id: "spotify",
      name: "Spotify",
      description: "Connect a Spotify account so teammates can see what music playing during a Team Jam — album art, track name, artist, and a live progress bar.",
      // When connected, card is NOT a link — disconnect is an explicit button below
      href: isSpotifyConnected ? null : "/api/spotify/connect",
      status: "active" as const,
      icon: (
      ...
      stats: [
        { label: "Used for", value: "Team Jam now-playing" },
      ],
      action: isSpotifyConnected
        ? { label: "Disconnect", href: "/api/spotify/disconnect", danger: true }
        : null,
      statusLabel: isSpotifyConnected ? "Connected" : "Not connected",
      statusColor: isSpotifyConnected ? "green" : "gray",
    },
    {
      id: "aws",
      name: "Amazon Web Services",
      description: "Connect your AWS account to link projects to their deployed infrastructure — EC2 instances, Amplify apps, CloudWatch logs, Route 53 DNS, and more.",
      href: "/integrations/aws",
      status: "active" as const,
      icon: (
        <svg viewBox="0 0 80 48" fill="none" className="w-7 h-5" aria-label="AWS">
          <path
      ...
      stats: [
        { label: "Status", value: isAwsConnected ? `Connected · ${awsRegion}` : "Not connected" },
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-black text-[#F0F0F0] flex items-center gap-3">
          <Plug2 size={22} className="text-[#F7941D]" />
          Integrations
        </h1>
        <p className="text-sm text-[#606060] mt-0.5">
          Connect external systems to bring your full tech stack into SmartHub.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-1 lg:grid-cols-2">
        {integrations.map((integration) => {
          ...
          // coming_soon cards get the dimmed treatment; all current cards are active
          const isDimmed = (integration.status as string) === "coming_soon";

          const card = (
            <div
              className={`group relative flex flex-col gap-4 p-5 rounded-2xl border bg-[#161616] transition-all duration-200 ${isDimmed
                ? "border-[rgba(255,255,255,0.04)] opacity-60 cursor-default"
                : isClickable
                  ? "border-[rgba(255,255,255,0.06)] hover:border-[rgba(247,148,29,0.25)] hover:bg-[#1A1A1A] cursor-pointer"
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-semibold text-[#F0F0F0] text-sm">{integration.name}</div>
                    <StatusBadge color={integration.statusColor} label={integration.statusLabel} />
                  </div>
                </div>
                {isClickable && (
                  <ChevronRight
                    size={16}
                    className="text-[#404040] group-hover:text-[#F7941D] transition-colors mt-0.5 flex-shrink-0"
                  />
                )}
              </div>

              {/* Description */}
              <p className="text-xs text-[#606060] leading-relaxed">{integration.description}</p>

              {/* Stats + optional action */}
              {(integration.stats.length > 0 || integration.action) && (
                <div className="flex items-center justify-between gap-4 pt-1 border-t border-[rgba(255,255,255,0.04)]">
                  <div className="flex gap-4">
                    {integration.stats.map((stat) => (
                      <div key={stat.label}>
                        <div className="text-xs text-[#404040]">{stat.label}</div>
                        <div className="text-sm font-semibold text-[#F0F0F0]">{stat.value}</div>
                      </div>
                    ))}
                  </div>
                  {integration.action && (
                    // Plain <a> intentionally — <Link> prefetches on scroll/hover which
                    // would silently fire a GET disconnect request as soon as the page loads.
                    <a
...
function StatusBadge({ color, label }: { color: string; label: string }) {
  const styles: Record<string, string> = {
    ...
    gray: "bg-[rgba(255,255,255,0.06)] text-[#606060]",
    orange: "bg-[rgba(247,148,29,0.12)] text-[#F7941D]",
  };
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${styles[color] ?? styles.gray}`}>
      {label}
    </span>
  );
}
